...
def transfer_to_xml(pic, txt, file_name, root):
    xml_save_path = os.path.join(root, "xml")  # 生成的xml文件存储的文件夹
...
if __name__ == '__main__':
    root = "data/VisDrone2019-DET-test-dev"
    t = time.time()
    print('Transfer .txt to .xml...ing....')
    txt_folder = os.path.join(root, "annotations")  # visdrone txt标签文件夹
    txt_file = os.listdir(txt_folder)
    img_folder = os.path.join(root, "images")  # visdrone 照片所在文件夹
    ...
    for txt in txt_file:
        ...
        try:
            transfer_to_xml(img_full_path, txt_full_path, txt.split('.')[0], root)
